...
def load_csv_file(airport_code, file_type_suffix):
    ...
    runs_to_try = []
    # Create a list of the last 3 model runs to try (current, -6h, -12h)
    for i in range(3):
        run_time = latest_update - timedelta(hours=i * 6)
        runs_to_try.append({
            'time': run_time,
            'url': construct_url(airport_code, run_time, file_type_suffix)
        })
